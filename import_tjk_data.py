#!/usr/bin/env python3
"""
Импортирует tjk_data.json с отображением прогресса.
Использование: python3 import_tjk_data.py [путь_к_json]
"""
import json
import os
import sys
...
def import_with_progress(json_file: Path):
    """Импортирует данные из JSON с отображением прогресса"""
    
    print("=" * 80)
    print("📦 ИМПОРТ ДАННЫХ ПО ТАДЖИКСКОМУ ЯЗЫКУ")
    print("=" * 80)
    ...
    if existing_subjects > 0 or existing_topics > 0 or existing_tasks > 0:
        ...
        response = input("\n❓ Продолжить импорт (добавить новые данные)? (yes/no): ").strip().lower()
        
        if response not in ['yes', 'y', 'да', 'д']:
            print("⚠️  Импорт отменён")
            return False
...
def main():
    json_file = Path(__file__).parent / 'tjk_data.json'
    
    if len(sys.argv) > 1:
        json_file = Path(sys.argv[1])
    
    success = import_with_progress(json_file)
    
    if success:
        print("\n🎉 Данные по таджикскому языку успешно импортированы!")
        print("\n💡 Следующие шаги:")
        print("   1. Проверьте данные: python3 manage.py shell")
        print("   2. Запустите сервер: python3 manage.py runserver")
    else:
